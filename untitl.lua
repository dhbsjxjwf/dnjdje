...
local window_esp = library.window("视觉")
local window_misc = library.window("混杂")
...
window_player.toggle("即时使用",false,function(val)
    flags.instapp = val
...
local walkspeedslider = window_player.slider("步行速度",16,25,1,16,function(val)
    hum.WalkSpeed = val
    flags.speed = val
end)
...
window_esp.toggle("门esp",false,function(val)
    ...
    if val then
        local function setup(room)
            local door = room:WaitForChild("门"):WaitForChild("Door")
...
window_esp.toggle("钥匙/拉杆esp",false,function(val)
    flags.espkeys = val
...
local entitynames = {"Rush大爹","Ambush大爹","地刺","A60","A120"}
        ...
        local function setup(room)
            if room.Name == "50" or room.Name == "100" then
                ...
                if figuresetup then
                    ...
                    local h = esp(fig,Color3.fromRGB(255,25,25),fig.PrimaryPart,"飞哥")
                    table.insert(esptable.entity,h)
                end 
            else
                local assets = room:WaitForChild("Assets")
...
window_esp.toggle("储物柜esp",false,function(val)
    flags.esplocker = val
...
window_esp.toggle("箱子esp",false,function(val)
    flags.espchest = val
...
window_esp.toggle("玩家esp",false,function(val)
    flags.esphumans = val
        ...
        local function check(v)
            if v:IsA("Model") then
                ...
                if goldvalue and goldvalue >= flags.goldespvalue then
                    local hitbox = v:WaitForChild("Hitbox")
                    local h = esp(hitbox:GetChildren(),Color3.fromRGB(255,255,0),hitbox,"金堆 [".. tostring(goldvalue).."]")
...
window_esp.slider("最低金值",5,150,5,25,function(val)
...
window_misc.toggle("无骨架门",false,function(val)
...
window_misc.toggle("自动图书馆密码",false,function(val)
    flags.getcode = val
...
window_misc.toggle("战利品光环",false,function(val)
